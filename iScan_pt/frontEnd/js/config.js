...
/**
 * modal interface
 * Created by zhaop on 2017/5/26.
 */
(function($){
    if (typeof $ === 'undefined') {
        throw new Error('cc.modal\'s javaScript requires jQuery')
    }
    ...
    Modal.DEFAULTS = {
        ...
        header: '<div class="header">' +
            ...
            '<button class="cc-minimize" data-type="min"></button>' +
            '<button class="cc-collapse" data-type="up"></button>' +
            '</div>'
    }
    ...
    Modal.prototype.show = function(){
        ...
        if(!self.isShown){
            setTimeout(function(){
                ...
                self.initialize();
                self.bindEvent();
                if(self.options.onOpen instanceof Function){
                    self.options.onOpen();
                }
                self.called = false;
            }, self.called ? self.duration : 0)
    ...
    Modal.prototype.hide = function(){
        if(this.isShown){
            ...
            if(this.options.modal){
                this.$element.parent('.cc-container').fadeOut(this.duration);
            }
    ...
    Modal.prototype.getInitSize = function(){
        var content = this.$element.find('.cc-content');
        if(!content.data('main.size')){
            content.data('main.size',{
                w: content[0].scrollWidth + content[0].offsetLeft * 2 + 10 ,
                h: content[0].scrollHeight + content[0].offsetTop + 10
            })
        }
        return content.data('main.size')
    }
    Modal.prototype.getInitPosition = function(){
        var initLeft = 0, initTop = 0;
        if(this.options.left && this.options.left.indexOf('px') > -1){
            initLeft = this.options.left;
        }else if(this.options.left && this.options.left.indexOf('%') > -1){
            initLeft = Math.floor(parseFloat(this.options.left) / 100 * (this.$element.parent().innerWidth() - this.$element.outerWidth())) + 'px';
        }
        if(this.options.top && this.options.top.indexOf('px') > -1){
            initTop = this.options.top;
        }else if(this.options.top && this.options.top.indexOf('%') > -1){
            initTop = Math.floor(parseFloat(this.options.top) / 100 * (this.$element.parent().innerHeight() - this.$element.outerHeight())) + 'px';
        }
        return {
            left: initLeft,
            top: initTop
    ...
    Modal.prototype.initialize = function(){
        ...
        if(this.options.modal){
            var container = this.$element.parent('.cc-container');
            if(!container[0]){
        ...
        // element
        if(!this.$element.hasClass('cc-modal')){
            this.$element.addClass('cc-modal fade');
        }
        this.$element.parent('.cc-container').show();
        this.$element.show();

        if(this.$element.find('.cc-content')[0]){
            var initSize = this.getInitSize();
            this.resize(this.$element, initSize.w, initSize.h);
        }else{
            this.resize(this.$element, 0, 0);
        }
        ...
        if(this.$element.children('.header').length == 0){
            if(this.$element.children().first()[0]){
                this.$element.children().first().before(this.options.header);
            }else{
                $(this.options.header).appendTo(this.$element)
            }
        }
        if(this.$element.children('.cc-resize').length == 0){
            $('<div class="cc-resize"></div>').appendTo(this.$element);
        }
    };

    Modal.prototype.computeMax = function(left, top){
        var self = this;
        var left = isNaN(left) ? self.$element.position().left : left,
            top = isNaN(top) ? self.$element.position().top : top;
        self.maxWidth = self.$element.parent().width() - left;
        self.maxHeight = self.$element.parent().height()- top;
        self.maxLeft = self.$element.parent().width() - self.$element.outerWidth();
        self.maxTop = self.$element.parent().height() - self.$element.outerHeight();
    }

    Modal.prototype.toggleContent = function(type){
        var self = this;
        self.$element.addClass('slide');
        if(type === 'up'){
            self.$collapse.attr('data-type', 'down');
            self.$collapse.data('collapse.height', self.$element.outerHeight());
            self.resize(self.$element, self.$element.outerWidth(), 0);
        }else{
            var initH = self.$collapse.data('collapse.height');
            if(!initH && initH !== 0){
                initH = self.getInitSize().h
            }
            self.$collapse.attr('data-type', 'up');
            self.resize(self.$element, self.$element.outerWidth(), initH);
        }
        setTimeout(function(){
            self.$element.removeClass('slide');
        }, 300)
    }
    Modal.prototype.toggleStatus = function(type){
        var self = this;
        if(type == 'min'){
            self.$collapse.attr('disabled', 'disabled');
            self.$header.off('mousedown.move.cc.modal');
            self.$resize.off('mousedown.resize.cc.modal');
        }else{
            self.$collapse.removeAttr('disabled');
            self.$header.on('mousedown.move.cc.modal', $.proxy(self.moveWindow, self));
            self.$resize.on('mousedown.resize.cc.modal', $.proxy(self.changeSize, self));
        }
    }
    Modal.prototype.toggleWindow = function(type){
        var self = this;
        self.$element.addClass('drop');
        if(type === 'min'){
            self.$minimize.attr('data-type', 'restore');
            ...
            self.resize(self.$element, 0, 0);
            self.move(self.$element, 10000, 10000, true);
        }else{
            var initSize = self.$minimize.data('minimize');
            self.move(self.$element, initSize.left, initSize.top);
            self.computeMax(initSize.left, initSize.top);
            self.resize(self.$element, initSize.width, initSize.height);
            self.$minimize.attr('data-type', 'min');
        }
        self.toggleStatus(type);
        setTimeout(function(){
            self.$element.removeClass('drop');
        }, 300)
    }
    Modal.prototype.changeSize =  function(e){
        ...
        if(e){
            ...
            var startX = e.pageX,
                startY = e.pageY,
                startW = self.$element.width(),
                startH = self.$element.height();
            $(document).on('mousemove.resize.cc.modal', function(e){
                self.resize(self.$element,
                        startW + e.pageX - startX,
                        self.$collapse.attr('data-type') === 'down'? 0 : startH + e.pageY - startY
                );
            })
            $(document).on('mouseup.resize.cc.modal', function(e){
                $(document).off('mousemove.resize.cc.modal');
                $(document).off('mouseup.resize.cc.modal');
            })
        }
    }
    Modal.prototype.moveWindow = function(e){
        var self = this;
        if(e){
            self.computeMax();
            var startX = e.pageX,
                startY = e.pageY,
                sLeft = self.$element.position().left,
                sTop = self.$element.position().top;
            $(document).on('mousemove.move.cc.modal', function(e){
                self.move(self.$element, e.pageX - startX + sLeft, e.pageY - startY + sTop);
            })
            $(document).on('mouseup.move.cc.modal', function(e){
                $(document).unbind('mousemove.move.cc.modal');
                $(document).unbind('mouseup.move.cc.modal');
            })
        }
    }
    Modal.prototype.bindEvent = function(){
        var self = this;
        self.$header = self.$element.find('.header');
        self.$resize = self.$element.find('.cc-resize');
        self.$collapse = self.$element.find('.cc-collapse');
        self.$minimize = self.$element.find('.cc-minimize');
        self.$close = self.$element.find('.cc-close');

        //active
        self.$element
            .off('mousedown.cc.modal')
            .on('mousedown.cc.modal', function(){
                $('.cc-modal').css({
        ...
        // move
        self.$header
            .off('mousedown.move.cc.modal')
            .on('mousedown.move.cc.modal', $.proxy(self.moveWindow, self));

        //resize
        self.$resize
            .off('mousedown.resize.cc.modal')
            .on('mousedown.resize.cc.modal', $.proxy(self.changeSize, self))

        if(self.options.collapsible){
            self.$collapse
                .off('click.collapse.cc.modal')
                .on('click.collapse.cc.modal', function(){
                    self.toggleContent(self.$collapse.attr('data-type'))
                });
        }else{
            self.$collapse.remove();
        }
        if(self.options.minimizable){
            self.$minimize
                .off('click.minimize.cc.modal')
                .on('click.minimize.cc.modal', function(){
                    self.toggleWindow(self.$minimize.attr('data-type'))
                });
        }else{
            self.$minimize.remove();
        }

        //close
        self.$close
            .off('click.close.cc.modal')
            .on('click.close.cc.modal', function(e){
                self.hide();
                if(self.options.onClose instanceof Function){
                    self.options.onClose();
                }
            });
    }
    Modal.prototype.move = function($obj, left, top, getCurrentMax){
        var self = this;
        if(self.maxLeft == null || self.maxTop == null){
            self.computeMax();
        }
        left = Math.min((+left || 0), self.maxLeft);
        top = Math.min(+top || 0, self.maxTop);
        if(getCurrentMax){
            left = self.$element.parent().width() - self.$element.outerWidth();
            top = self.$element.parent().height() - self.$element.outerHeight();
            $obj.css({
                left: left + 'px',
                top: top + 'px'
            })
        }else{
            left >= 0 && left <= self.maxLeft ? $obj.css({left: left + 'px'}) : false;
            top >= 0 && top <= self.maxTop ? $obj.css({top: top + 'px'}) : false;
        }
    }
    Modal.prototype.resize = function($obj, width, height){
        var self = this;
        if(self.maxWidth == null || self.maxHeight == null){
            self.computeMax();
        }
        var minW = self.options.minWidth,
            minH = self.options.minHeight,
            maxW = self.maxWidth,
            maxH = self.maxHeight;
        width = Math.max((+width || 0), minW);
        height = Math.max(+height || 0, minH);
        width = Math.min((+width || 0), maxW);
        height = Math.min(+height || 0, maxH);
        width >= minW && width <= maxW ? $obj.outerWidth(width) : false;
        height >= minH && height <= maxH ? $obj.outerHeight(height) : false;
    }
    var Plugin = function(option){
        return this.each(function () {
            var $this   = $(this)
            var data    = $this.data('cc.modal')
            var options = $.extend({}, Modal.DEFAULTS, $this.data(), typeof option == 'object' && option)
            if (!data){
                $this.data('cc.modal', (data = new Modal(this, options)))
            }
