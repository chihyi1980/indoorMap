...
// atlas version of this.svgCanvasToString
// executed when update | save svg to database
this.svgCanvasToAtlasString = function() {

	return this.svgToAtlasString(svgcontent, 0);
    //var output = this.svgToString(svgcontent, 0);
	// console.log(output);
}
this.svgToAtlasString = function(elem, indent) {
	...
	if (elem && elem.id === 'svgcontent') {
		...
		var atlasDoms = gDom.childNodes;

		var floor,
			shops = [];

		for (var i = 0; i < atlasDoms.length; i++) {
			var atlasDom = atlasDoms.item(i);
			if(atlasDom.nodeType !== 1) continue;
			// ignore none-path & none-rect nodes
			var nodeName = atlasDom.nodeName,
				sid = atlasDom.getAttribute('sid'),
				className = atlasDom.getAttribute('class');

			if(nodeName !== 'path' && nodeName !== 'rect') continue;

			var isFloor = className.indexOf('floor') >= 0,
				isShop = className.indexOf('shop') >= 0;

			if (!isFloor && !isShop) continue;

			var type = atlasDom.nodeName,
				isPath = type === 'path',
				isRect = type === 'rect';

			function shapeResolver(attr) {
				var shape;
				if (isPath) { 
					shape = attr.d;
				} else if (isRect) {
					...
					for (var k in attr) {
						if (k === 'x' || k === 'y' || k === 'width' || k ==='height')
							attr_arr.push(k + '="' + attr[k] + '"');
					}
					shape = attr_arr.join(' ');
				}
				return shape;
			}
			function styleResolver(attr) {
				var style,
					attr_arr = [];

				for (var k in attr) {
					if (isPath && isValueableKeyForPath(k)) continue;
					if (isRect && isValueableKeyForRect(k)) continue;
					attr_arr.push(k + '="' + attr[k] + '"');
				}
				style = attr_arr.join(' ');
				console.log(style);
			...
			function isValueableKeyForPath(key) {
				return /d|class|id/.test(key);
			}
			function isValueableKeyForRect(key) {
				return /id|class|x|y|width|height/.test(key);
			}

			var attr_obj = {};	
			for (var j=0; j<atlasDom.attributes.length; j++) {
				attr_obj[atlasDom.attributes.item(j).name] = atlasDom.attributes.item(j).value;
			}

			var atlas_obj = {
				type: type,
				shape: shapeResolver(attr_obj),
				style: styleResolver(attr_obj)
			};

			if (!!sid) atlas_obj['sid'] = sid;

			if (isFloor) {
				floor = atlas_obj;
			} else if (isShop) {
				atlas_obj.shapeType = 1;
				if (className.indexOf('not-accessible') >= 0)
					atlas_obj.shapeType = 0;
				shops.push(atlas_obj);
			}
		}
...
// 修改自 importSvgString
// 忽略了viewBox属性
this.importAtlasSvgString = function(xmlString) {

    try {

        // xml head and annotation filter
        ...
        // 重画w/h
        svgCanvas.setResolution(innerw, innerh);

        var nodes = svg.firstChild;

        while(nodes.firstChild){
            var node = nodes.firstChild;
            node.id = getNextId();

            var styleFloor = svgEditor.curConfig.initFloor;
            var styleShop  = svgEditor.curConfig.initShop;
            var styleDisable = svgEditor.curConfig.initDisable;

            var fill_str = styleShop.fill;
            var stroke_str = styleShop.stroke;
            var stroke_width_str = styleShop.stroke_width;

            // 不可行区域
            if(node.getAttribute('class').indexOf('not-accessible') >= 0) {
                fill_str = styleDisable.fill;
                stroke_str = styleDisable.stroke;
            }

            // floor
            if(node.getAttribute('class').indexOf('floor') >= 0) {
                fill_str = styleFloor.fill;
                stroke_str = styleFloor.stroke;
                if(!node.getAttribute('fill-opacity')) node.setAttribute('fill-opacity', styleFloor.fill_opacity);
                stroke_width_str = styleFloor.stroke_width;
            }

            // 添加默认样式
            if(!node.getAttribute('fill')) node.setAttribute('fill', fill_str);
            if(!node.getAttribute('stroke')) node.setAttribute('stroke', stroke_str);
            if(!node.getAttribute('stroke-width')) node.setAttribute('stroke-width', stroke_width_str);

            getCurrentDrawing().getCurrentLayer().appendChild(nodes.firstChild);
        }
...
this.setResolution = function(x, y, inject) {
	var res = getResolution();
	var w = res.w, h = res.h;
	var batchCmd;
...
// hack setBackground
this.setAtlasBackground = function(color, img) {
    var bg = svgedit.utilities.getElem('canvasBackground');
    var border = $(bg).find('rect')[0];
    var bg_img = svgedit.utilities.getElem('background_image');
    border.setAttribute('fill', color);

    if (bg_img) bg_img.parentNode.removeChild(bg_img);
