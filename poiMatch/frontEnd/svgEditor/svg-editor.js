...
(function() {
	...
	window.svgEditor = (function($) {
		...
		var svgCanvas, urldata,
			...
			*/
			...
			defaultConfig = {
                ...
                // customize, dont know if it's proper to add config here
                pid: null, // atlas floor poi_id, default to null, load from urldata
                initShop: null,
                initFloor: null,
                initDisable: null
			},
			...
			*/
			uiStrings = editor.uiStrings = {
				...
				notification: {
                    ...
                    elemImport: 'Do you want to import a new SVG?\nThis will clear everything!'
				}
			};
        ...
        function loadAtlasSvgString(str, callback) {
            var success = svgCanvas.importAtlasSvgString(str) !== false;
            callback = callback || $.noop;
            if (success) {
                callback(true);
            } else {
		...
		*/
		...
		editor.init = function () {
			...
			var supportsNonSS, resize_timer, changeZoom, Actions, curScrollPos,
                ...
                // div#workarea
				workarea = $('#workarea'),
                // canvas菜单栏: duplicate, delete, ... [top工具栏]
            ...
            // push to atlas
            var showAtlasSourceEditor = function(e, forSaving) {
                var contentResult = svgCanvas.svgCanvasToAtlasString();

                console.log(contentResult);

                var pid = editor.curConfig.pid;

                if(!pid) return alert('poi_id not found !!');

                // push update to server
                $.ajax({
                    url: ATLAS_POI_CONFIG.EDITOR_POST_URL + '/' + pid,//'/api/mapeditor/svg/' + pid,
                    dataType: 'json',
                    type: 'POST',
                    data: contentResult,
                    success: function(data){
                        console.log(data);
                        alert('save content successfully .');
                    },
                    error: function() {
			...
			var updateToolbar = function() {
                ...
                // atlas class assigner & sid assigner
                if (selectedElement != null) {
                    var assigner = $('#tool_class_assign'),
                    	sidAssigner = $('#tool_sid_assign');
                    if(selectedElement.tagName === 'path') {
                        assigner.prop('disabled', false);
                        var className = selectedElement.getAttribute('class'),
                        	sid = selectedElement.getAttribute('sid');
                        // assigner
                        switch(className) {
                            case 'floor': assigner.val('floor'); break;
                            case 'shop': assigner.val('shop'); break;
                            case 'shop not-accessible': assigner.val('not-accessible'); break;
                            default: assigner.val('nothing');
                        }
                        // sidAssigner
                        sidAssigner.val(sid);
                    } else {
                        assigner.prop('disabled', true);
                    }
                }

				updateToolButtonState();
			};
            // update class of selectedElement
            $('#tool_class_assign').change(function(){
                var newClassName = $(this).val();
                var className = '';
                switch(newClassName) {
                    case 'floor': className = 'floor'; break;
                    case 'shop': className = 'shop'; break;
                    case 'not-accessible': className = 'shop not-accessible'; break;
                    default: className = null;
                }
                if(className) selectedElement.setAttribute('class', className);
            });
            // update sid of selectedElement
            // update sid of selectedElement
            $('#tool_sid_assign').blur(function () {
            	var sid = $(this).val();
            	console.log('-- on blur, %s --', sid);
            	if (!selectedElement) return;
            	selectedElement.setAttribute("sid", (sid + '').trim());
            });
            $('#tool_sid_assign').on('input propertychange', function () {
                var sid = $(this).val();
                console.log('-- on blur, %s --', sid);
                if (!selectedElement) return;
                selectedElement.setAttribute("sid", (sid + '').trim());
...
//					$('.contextMenu').hide();
			...
			*/
            ...
            //var input_bg_img = document.getElementById('input_bg_img');
            //input_bg_img.addEventListener('change', function(e){
            ...
            //        svgCanvas.clear();
            //        svgCanvas.setResolution(bg_img.width, bg_img.height);
            //        svgEditor.setConfig({
            //            dimentions: [bg_img.width, bg_img.height]
            //        });
            //        zoomImage();
            //        populateLayers();
            //        updateContextPanel();
            ...
            var clickImportBg = function() {
                //svgCanvas.setAtlasBackground('#fff', );
            }
			...
			if (svgedit.browser.isMac() && !window.opera) {
				var shortcutButtons = ['tool_bg_img', 'tool_clear', 'tool_save','tool_source', 'tool_undo', 'tool_redo', 'tool_clone'];
...
'					<defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml');
		...
		//	}
			...
			Actions = (function() {
				...
				var tool_buttons = [
                    ...
                    {sel: '#tool_bg_img', fn: setBackgroundImage, evt: 'mouseup'},
                    ...
                    // 保存
                    {sel: '#tool_update', fn: showAtlasSourceEditor, evt: 'mouseup', key: ['S', true]},
					{sel: '#tool_export', fn: clickExport, evt: 'mouseup'},
					{sel: '#tool_open', fn: clickOpen, evt: 'mouseup', key: ['O', true]},
					{sel: '#tool_import', fn: clickImport, evt: 'mouseup'},
                    // 背景图片导入
                    {sel: '#tool_import_bg', fn: clickImportBg, evt: 'mouseup'},
					{sel: '#tool_source', fn: showSourceEditor, evt: 'click', key: ['U', true]},
                    //{sel: '#tool_source', fn: showAtlasSourceEditor, evt: 'click', key: ['U', true]},
		...
		//			{sel: '#tools_ellipse_show', fn: clickEllipse, evt: 'click'},
            ...
            // 导入SVG open-new-page -> import svg file -> render svg
			if (window.FileReader) {
                    ...
                    if (file.type.indexOf('image') != -1) {
                        ...
                        if (file.type.indexOf('svg') != -1) {
                            ...
                            reader.onloadend = function(e) {
                                // ignore refered svg
                                svgCanvas.importAtlasSvgString(e.target.result, true);
                ...
                // 当前svg不为空(w/h不为0)，则使用当前文档的w/h
                // 当前svg为空 使用图片的w/h
                var importBackgroundImage = function(e) {
                    e.stopPropagation();
                    e.preventDefault();
                    $('#workarea').removeAttr('style');
                    $('#main_menu').hide();
                    ...
                    if (file.type.indexOf('image') != -1) {
                        ...
                        reader.onloadend = function(e) {
                            ...
                            img.onload = function() {
                                ...
                                // set background
                                svgCanvas.setAtlasBackground('#fff', img);
                            };
                        };
                        reader.readAsDataURL(file);
                    }
				...
				var open = $('<input type="file">').change(function() {
                    ...
                    // here goes customized
                    // equals refresh this page before import anything
                    alert('running');
                    clickClear();
				...
				var imgImport = $('<input style="display: none;" type="file">').change(importImage);
                var bgImport = $('<input style="display: none;" type="file">').change(importBackgroundImage);

                $("#tool_import").click(function(e){
                    var dims = curConfig.dimensions;
                    $.confirm(uiStrings.notification.elemImport, function(ok) {
                        if (!ok) {return;}
                        setSelectMode();
                        svgCanvas.clear();
                        svgCanvas.setResolution(dims[0], dims[1]);
                        ...
                        // import as atlas svg file
                        imgImport.click();
                    });
                });

                $("#tool_import_bg").click(function(e){
                    //var dims = curConfig.dimensions;
                    //setSelectMode();
                    //svgCanvas.clear();
                    //svgCanvas.setResolution(dims[0], dims[1]);
                ...
                $('#tool_import').show();
                $('#tool_import_bg').show();
                $('body').prepend(imgImport);

			} else {
                alert('HTML5 File Reader Features not support !');
            }
        ...
        // load atlas svg string
        editor.loadFromAtlasString = function(str) {
            editor.ready(function() {
               loadAtlasSvgString(str);
                // 适应层内容
			...
			editor.ready(function() {
				$.ajax({
					...
					success: function(str) {
                        ...
                        // hack
                        // same as 'import as svg'
                        svgCanvas.importAtlasSvgString(str, true);
