	...
	window.svgEditor = (function($) {
		...
		*/
		...
		Editor.setConfig = function(opts, noOverride) {
			...
			if (extensions) {
				curConfig.extensions = opts.noDefaultExtensions && curConfig.extensions ? extensions : curConfig.extensions.concat(extensions);
...
//					}
			...
			var extAdded = function(win, ext) {
				...
				if (ext.langReady) {
					if (curConfig.langChanged) { // We check for this since the "lang" pref could have been set by storage
						var lang = $.pref('lang');
...
//					$('.contextMenu').hide();
		...
		//		if (url) url = url[1];
			...
			if (curConfig.prefsLoaded) {
				savePreferences(); // Change behavior per local storage
			}
