                        ...
                        dateList.forEach(function(date){
                            ...
                            otherIds.forEach(function(userId){
                                Tagger.remove({userId: userId, enterTime : {$gt: begin, $lt: end}}, function(err, ret) {
                                    ...
                                    buildTaggers(date + ' 08:40:00',date + ' 09:20:00', 5, 10, userId, iscans, iscansMap);

                                    buildTaggers(date + ' 09:25:00',date + ' 10:40:00', 8, 20, userId, iscans, iscansMap);

                                    buildTaggers(date + ' 11:00:00',date + ' 11:40:00', 5, 5, userId, iscans, iscansMap);

                                    buildTaggers(date + ' 12:10:00',date + ' 16:40:00', 8, 20, userId, iscans, iscansMap);

                                    buildTaggers(date + ' 17:00:00',date + ' 17:35:00', 5, 10, userId, iscans, iscansMap);
 ...
 * range1,rang2: int  时间跨度，设5～20之间
 * userId: string 需要生成数据的用户
 * iscans: array iscanId的数组
 * iscansMap : object  {iscanId：[jobsiteId, beaconId]}
 */
function buildTaggers(begin, end, range1, range2, userId, iscans, iscansMap){
    var minute = 1000 * 60;
    while (begin < end) {
        begin = new Date(new Date(begin).getTime() +
            Math.floor(Math.random() * range1 + 4) * minute +
        ...
        iscans.forEach(function (iscanId) {
            var newTagger = Tagger({
                ...
                enterTime: new Date(new Date(begin).getTime() +
                    Math.floor(Math.random() * range2 + 4) * minute +
